...
export default class DepartsAdd extends React.Component {
  ...
  onSubmit(e) {
    ...
    if (
      ...
      Meteor.call("depenses.insert", dateTime, "DCB", imm.trim().toUpperCase(), "Carburant bus", 635, parseInt(fuel.trim()), "RAS", fuel.trim() > 0 ? true : false, (err, res) => {
            if (!err) {Meteor.call("departs.insert", res, dateTime, imm.trim().toUpperCase(), dest.trim().toLocaleLowerCase(), driver.trim().toLocaleLowerCase(), parseInt(fuel), parseInt(fdr), parseInt(amount), parseInt(seat), parseInt(leasing), parseInt(km), parseInt(eau) , parseInt(cafe), parseInt(the), parseInt(sucre), parseInt(cuillere), parseInt(gobelet), parseInt(couvercle), parseInt(sandwich), parseInt(croissant), parseInt(choco), parseInt(biscuit), parseInt(mouchoir), obs.trim(), (err, res) => {
                  if (!err) {

                    this.handleClose();
                    Bert.alert("enregistrements ajoute avec succes.", "danger", "growl-top-right", "fa-check");
