...
export class DepartsList extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    // Filtre
    const filtreMultipleDeparts = filtrage(
      departs,
      searchClasse,
      searchVille,
      searchBus,
        ...
        <Table selectable singleLine>
          <Table.Header>
            <Table.Row>
              ...
              <Table.HeaderCell>Eau</Table.HeaderCell>
              <Table.HeaderCell>Cafe</Table.HeaderCell>
              <Table.HeaderCell>The</Table.HeaderCell>
              <Table.HeaderCell>Sucre</Table.HeaderCell>
              <Table.HeaderCell>Cuil.</Table.HeaderCell>
              <Table.HeaderCell>Gob.</Table.HeaderCell>
              <Table.HeaderCell>Couv.</Table.HeaderCell>
              <Table.HeaderCell>Sand.</Table.HeaderCell>
              <Table.HeaderCell>Croiss.</Table.HeaderCell>
              <Table.HeaderCell>Choco.</Table.HeaderCell>
              <Table.HeaderCell>Bisc.</Table.HeaderCell>
              <Table.HeaderCell>Mouch.</Table.HeaderCell>
          ...
          <Table.Body>
            {this.props.departs.length === 0 ? (
              <DepartsEmptyListItem text="No Items, if is an unexpected result please contact the admin" />
            ) : (
              undefined
            )}
            {this.props.loading && !!this.props.departs.length ? (
              <DepartsEmptyListItem text="Loading Data , please wait ..." />
            ) : (
              undefined
            )}
            {!!this.props.departs.length && !this.props.loading
              ? this.props.Session.get("departsFiltered").map(depart => {
                  return <DepartsListItem key={depart._id} depart={depart} />;
                })
              : undefined}
          </Table.Body>
        </Table>
...
export default createContainer(() => {
  ...
  const departStartedDate =
    Session.get("departStartedDate") || new Date().setHours(0, 0, 0, 0);
  const departEndedDate =
    Session.get("departEndedDate") || new Date().setHours(23, 59, 0, 0);

  const departsHandle = Meteor.subscribe(
    "departs",
    departStartedDate,
    departEndedDate
  );
  ...
  return {
    ...
    searchVille,
    searchBus,
    departs: Departs.find({ visible: true })
      .fetch()
      .map(depart => {
        return {
          ...
          total:
            depart.leasing === 0 ? depart.amount * depart.seats : depart.leasing
        };
      })
  };
}, DepartsList);
